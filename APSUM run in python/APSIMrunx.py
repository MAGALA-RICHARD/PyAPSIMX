...
def detectAPSIMX():
  try:
      ...
      if len(apasim_folder) ==0:
        for rr, dd, ff in os.walk(r'C:\Users\rmagala\AppData\Local\Programs'):
         for d in dd:
          if d.startswith("APSIM"):
            apasim_folder.append(opj(rr, d))
      ...
      if 'C:\\Users\\rmagala\\AppData\\Local\\Programs\\APSIM2022.12.7130.0' in apasim_folder:
       extString = fdir[0].split('C:\\Users\\rmagala\\AppData\\Local\\Programs')[1]
      else:
  ...
  # APSIM next generation root directory regsitry
      ...
      if os.path.isfile(path):
        print("Congratulations APSIM detected")
...
def runAPSIM2(completepath,  apsimxlocal =apsimloc, readdb = True, select_report=None, ):
    ...
    try:
        start = time.perf_counter()
        if not completepath.endswith(".apsimx") and not os.path.isfile(completepath):
           print("filename is empty or doesnot exists. did you forget to write the apsimx filename?")
        else:
           ...
           if os.path.isfile(apsimfilepath) == False:
               ...
               # join the two directory strings
                ...
                resultMessage
                if readdb:
                   newfilenamedb = completepath.split("apsimx")[0] + "db"
